...
class GamepadService : Service() {
    ...
    /**
     * Modo transparente: el servicio corre pero NO intercepta ninguna tecla. Se activa cuando
     * automatizacion=ON y la app actual no tiene perfil asignado.
     */
    ...
    companion object {
        ...
        const val ACTION_RELOAD_ACTIVE_PROFILE = "com.t9mapper.RELOAD_ACTIVE_PROFILE"
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ...
            ACTION_RELOAD_ACTIVE_PROFILE -> {
                // El usuario guardó cambios en el perfil activo desde el editor.
                // Releer desde Room y reenviar al daemon sin cambiar qué perfil está activo.
                serviceScope.launch {
                    val profile = activeProfile ?: return@launch
                    val refreshed = db.profileDao().getProfileById(profile.id) ?: return@launch
                    loadProfile(refreshed)
                }
            }
        }
        return START_STICKY
    ...
    /**
     * Si el AccessibilityService está en Crashed services, lo reactiva via root. Usa la API de
     * settings para desactivarlo y reactivarlo.
     */
    ...
    /**
     * Modo transparente: la app actual no tiene perfil asignado y la automatización está encendida.
     * El servicio sigue corriendo (dispositivo uinput activo) pero no intercepta ninguna tecla — el
     * teclado funciona exactamente como de fábrica.
     */
    private fun handleTransparent() {
        ...
        GamepadNative.resetAxes(uinputFd)
        stopRamp()
        // Soltar el teclado físico si el daemon lo tenía capturado
        if (uinputFd > 0) {
            GamepadNative.sendRawToSocket(uinputFd, byteArrayOf(0xFD.toByte(), 0x00.toByte()))
        }
        updateNotification()
    }
    ...
    /**
     * Copia el binario uinput_helper desde los assets de la app a un directorio ejecutable en el
    ...
    private fun installHelper(): String? {
        return try {
            ...
            Log.d(
                    "GamepadService",
                    "Helper extraído: ${helperFile.absolutePath} (${helperFile.length()} bytes)"
            )
    ...
    private fun requestRootAndOpenUinput(): Boolean {
        return try {
            ...
            val process = ProcessBuilder("su", "-c", helperPath).redirectErrorStream(true).start()
            ...
            if (sock > 0) {
                uinputFd = sock
                // El daemon destruye el dispositivo al cerrar el socket anterior (goto done).
                // Al reconectar hay que recrearlo según el perfil activo.
                val profile = activeProfile
                if (profile != null && profile.deviceType != 0) {
                    GamepadNative.sendRawToSocket(
                        uinputFd,
                        byteArrayOf(0xFC.toByte(), profile.deviceType.toByte())
                    )
                }
                return true
            }
    ...
    private suspend fun loadProfile(profile: Profile) {
        ...
        // 1. Configurar tipo de dispositivo (0xFC) y grab (0xFD) en el daemon
        applyProfileToDaemon(profile)

        // 2. Enviar tabla de mapeos (0xFE) solo si no es perfil Default
        if (profile.deviceType != 0) {
            sendMappingsToHelper(currentMappings, profile)
        }

        withContext(Dispatchers.Main) { updateNotification() }
    }

    private fun sendMappingsToHelper(mappings: List<KeyMapping>, profile: Profile) {
        if (profile.deviceType != 0) {
            if (uinputFd < 0 || mappings.isEmpty()) return
            try {
                val validMappings = mappings.take(64)
                // Protocolo: [0xFE][n] + por mapping: [kc_lo][kc_hi][btn_lo][btn_hi][type][ax_b0][ax_b1][ax_b2][ax_b3]
                // 9 bytes por mapping — axis_value serializado completo en 4 bytes (int32 LE)
                val buf = ByteArray(2 + validMappings.size * 9)
                buf[0] = 0xFE.toByte()
                ...
                validMappings.forEachIndexed { i, m ->
                    ...
                    buf[base + 7] = ((m.axisValue shr 16) and 0xFF).toByte()
                    buf[base + 8] = ((m.axisValue shr 24) and 0xFF).toByte()
                }
    ...
    fun onKeyEvent(keyCode: Int, action: Int): Boolean {
        ...
        if (mapping == null) {
            Log.d(
                    "GamepadService",
                    "onKeyEvent keyCode=$keyCode sin mapeo (mappings=${currentMappings.size})"
            )
            return false
        }
        ...
        Log.d(
                "GamepadService",
                "onKeyEvent keyCode=$keyCode pressed=$pressed type=${mapping.mappingType}"
        )
    ...
    private fun keyCodeToDPadDirection(keyCode: Int): Int =
            when (keyCode) {
    ...
    private fun buildNotification(): Notification {
        ...
        val builder =
                NotificationCompat.Builder(this, T9GamepadApp.NOTIFICATION_CHANNEL_ID)
        ...
        if (state == State.RUNNING) {
            val pauseIntent =
                    PendingIntent.getService(
                            this,
                            1,
                            Intent(this, GamepadService::class.java).apply {
                                action = ACTION_PAUSE
                            },
    ...
    private fun applyProfileToDaemon(profile: Profile) {
        val socket = uinputFd // Tu variable que mantiene el socket fd
        if (socket < 0) return

        // 1. Decidir estado de Grab y Tipo de Dispositivo
        // deviceType: 0 = Default, 1 = Xbox, 2 = Teclado PC
        val devType = profile.deviceType
        val grabState = if (devType == 0) 0 else 1 // Si es 0 (Default), soltamos el teclado

        // 2. Enviar comandos de control al daemon en C
        // Comando 0xFD: Grab/Ungrab teclado físico
        GamepadNative.sendRawToSocket(socket, byteArrayOf(0xFD.toByte(), grabState.toByte()))

        // Comando 0xFC: Cambiar dispositivo virtual
        GamepadNative.sendRawToSocket(socket, byteArrayOf(0xFC.toByte(), devType.toByte()))

        // Si es perfil por defecto (0), no enviamos mapeos, aquí terminamos.
        if (devType == 0) {
            Log.d("GamepadService", "Perfil Default aplicado: Teclado liberado y mapeos ignorados.")
            return
        }

        // 3. (Aquí llamas a tu lógica original que envía los mappings con 0xFE)
